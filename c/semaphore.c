...
#define CANNOT_WAIT_READYTOSTART 10
#define CANNOT_INCREMENT_DONEPROCESSING 11
#define CANNOT_ALLOCATE_SEMAPHORES 12
#define CANNOT_WAIT_FOR_THREAD 13
#define CANNOT_KICKOFF_THREAD 14
...
//          fprintf(stderr, "interesting: able to allocate buffer[%zu] for %zu bytes\n", i, sizeof **ThreadInfo);/g
          ...
          {
            fprintf(stderr, "error: unable to create semaphores for thread %zu\n", i);
            ErrorStatus = CANNOT_ALLOCATE_SEMAPHORES;
          } 
          else
          {
